...
- [ ] **CINF-01**: Scenario content defined as typed TypeScript data objects separated from React components (ScenarioConfig interface)
...
- [ ] **CINF-03**: Zod schema validation for all scenario content at build/dev time — prevents silent content bugs across 168 scenario slots (8 roles × 3 phases × 7 friction scenarios)
- [ ] **CINF-04**: Debrief template structure enforced as non-negotiable in schema (what happened / why / how / who you became / tomorrow's action)
- [ ] **CINF-05**: Each scenario choice carries a `crossRoleImpact[]` array keyed to specific decisions — no hardcoded consequence strings
