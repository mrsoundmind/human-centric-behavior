    ...
    return (
        <div className="min-h-screen bg-background text-foreground flex flex-col items-center justify-center p-6 md:p-12 relative overflow-hidden font-sans">
            ...
            <div className="max-w-5xl w-full z-10 relative">
                ...
                >
                    <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-muted border border-border backdrop-blur-md mb-8">
                        <UserCircle2 className="w-4 h-4 text-primary" />
                        <span className="text-[10px] font-mono uppercase tracking-[0.2em] text-muted-foreground">Step 3: Identity Calibration</span>
                    </div>

                    <h1 className="text-4xl md:text-6xl font-black tracking-tight leading-tight mb-6">
                        Under pressure,<br />
                        <span className="text-transparent bg-clip-text bg-gradient-to-r from-foreground to-muted-foreground">what do you automatically protect?</span>
                    </h1>
                        ...
                        return (
                            <motion.button
                                ...
                                className={`text-left p-6 rounded-2xl border transition-all duration-500 relative overflow-hidden group ${isSelected
                                        ? "bg-card border-primary/30 scale-[1.02]"
                            ...
                            >
                                ...
                                <div className="flex items-center gap-4 mb-4 relative z-10">
                                    <div className={`p-3 rounded-xl transition-colors duration-300 ${isSelected ? `bg-gradient-to-br ${instinct.color} text-primary-foreground` : 'bg-muted text-muted-foreground group-hover:text-foreground'}`}>
                                        {instinct.icon}
                                    </div>
                                    <h3 className={`text-xl font-bold transition-colors duration-300 ${isSelected ? 'text-foreground' : 'text-muted-foreground'}`}>{instinct.label}</h3>
                                </div>

                                <p className="text-sm text-muted-foreground leading-relaxed relative z-10 mb-4 h-10">
                                    ...
                                    {isSelected && (
                                        ...
                                        >
                                            <p className="text-sm font-mono text-primary italic">
                                                <span className="text-primary font-bold not-italic">Hidden Truth: </span>
                    ...
                    <AnimatePresence>
                        {selectedId && (
                            ...
                            >
                                <button
                                    onClick={onContinue}
                                    className="group relative px-10 py-5 bg-primary text-primary-foreground rounded-full font-black text-sm uppercase tracking-[0.2em] transition-all hover:scale-105 overflow-hidden shadow-lg"
                                >
