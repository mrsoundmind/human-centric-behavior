...
export const SystemReaction = ({ choice = 'defensive', onContinue }: SystemReactionProps) => {
    ...
    const mainColor = isDefensive ? 'red' : 'orange';
    ...
    const renderCard = (icon: any, title: string, desc: string, index: number) => {
        ...
        return (
            ...
            >
                <div className={`absolute inset-0 bg-gradient-to-br from-${mainColor}-500/20 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 rounded-2xl blur-xl`} />
                <div className={`bg-[#0a0a0c] border border-white/[0.05] hover:border-${mainColor}-500/30 p-8 rounded-2xl relative z-10 transition-colors h-full flex flex-col items-center text-center`}>
                    <div className={`w-14 h-14 rounded-2xl bg-${mainColor}-500/10 text-${mainColor}-400 flex items-center justify-center mb-6 shadow-[0_0_20px_rgba(239,68,68,0.15)]`}>
    ...
    return (
        <div className="min-h-screen bg-[#030305] text-white flex flex-col items-center justify-center p-6 md:p-12 relative overflow-hidden font-sans">
            ...
            <div className={`absolute inset-0 bg-[radial-gradient(circle_at_center,rgba(${isDefensive ? '239,68,68' : '249,115,22'},0.15),transparent_60%)] pointer-events-none blur-[100px]`} />
            ...
            <div className="max-w-5xl w-full z-10 relative flex flex-col items-center">
                ...
                >
                    <div className={`inline-flex items-center gap-2 px-4 py-2 rounded-full border mb-6 backdrop-blur-md ${isDefensive ? 'bg-red-500/10 border-red-500/20 text-red-400' : 'bg-orange-500/10 border-orange-500/20 text-orange-400'
                        }`}>
                    ...
                    <h1 className="text-5xl md:text-7xl font-black tracking-tight mb-6">
                        ...
                        <br />
                        <span className={`text-transparent bg-clip-text bg-gradient-to-r ${isDefensive ? 'from-red-400 to-red-600' : 'from-orange-400 to-orange-600'}`}>
                ...
                {step === 0 && (
                    <motion.button
                        ...
                        whileTap={{ scale: 0.95 }}
                        className={`group relative px-12 py-5 bg-white text-black rounded-full font-black text-sm uppercase tracking-[0.2em] shadow-[0_0_40px_rgba(255,255,255,0.1)] transition-all`}
                        onClick={() => setStep(1)}
                    >
                ...
                <div className="h-32 mt-12 flex flex-col items-center justify-center gap-4">
                    {step === 1 && (
                        <motion.button
                            ...
                            onClick={() => setStep(2)}
                            className={`px-6 py-2 rounded-full border border-white/10 text-xs font-mono text-gray-400 hover:text-white hover:border-${mainColor}-500/50 transition-all uppercase tracking-widest bg-white/[0.02]`}
                        >
                            Reveal Next Impact
                        </motion.button>
                    )}
                    ...
                    {step >= 3 && (
                        <motion.button
                            ...
                            animate={{ opacity: 1, y: 0 }}
                            onClick={onContinue}
                            className="group relative px-10 py-5 bg-white text-black rounded-full font-black text-sm uppercase tracking-[0.2em] shadow-[0_0_40px_rgba(255,255,255,0.1)] hover:scale-105 transition-all"
                        >
