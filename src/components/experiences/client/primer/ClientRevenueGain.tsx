...
export const ClientRevenueGain = ({ onNext }: ClientRevenueGainProps) => {
    ...
    return (
        <div className="max-w-5xl mx-auto py-12 px-6">
            ...
            >
                <div className="inline-block px-4 py-1.5 rounded-full bg-green-500/10 border border-green-500/20 text-xs font-bold tracking-widest text-green-500 font-mono">
                    PHASE 5: THE UPSIDE
                </div>
                <h1 className="text-5xl md:text-7xl font-display font-light text-white">
                    The UX Dividend. <br />
                    <span className="font-bold text-green-500 text-6xl md:text-8xl">+${recoveredRevenue.toLocaleString()}</span> <span className="text-2xl text-gray-500">/ mo</span>
                </h1>
                <p className="text-gray-400 max-w-2xl mx-auto">
                    This is not magic. This is simply <span className="text-white font-bold">Removing Friction</span>.
                </p>
            </motion.div>

            {/* 1. CALCULATOR GRID */}
            <div className="grid md:grid-cols-2 gap-8 bg-[#0F172A] p-8 rounded-3xl border border-white/10 relative overflow-hidden mb-12">
                {/* Background Glow */}
                <div className="absolute top-0 right-0 w-64 h-64 bg-green-500/10 blur-[100px] rounded-full pointer-events-none"></div>

                {/* Comparison Chart */}
                <div className="space-y-6">
                    <h3 className="text-xl font-bold text-white">Survival Comparison</h3>
                    ...
                    <div className="space-y-4 font-mono text-sm">
                        ...
                        <div className="relative h-2 bg-gray-800 rounded-full overflow-hidden">
                            <div className="absolute top-0 left-0 h-full bg-red-500" style={{ width: `${(1 - LOSS_RATES_BAD.bait) * 100}%` }}></div>
                            <div className="absolute top-0 left-0 h-full bg-green-500 opacity-50" style={{ width: `${(1 - LOSS_RATES_GOOD.bait) * 100}%` }}></div>
                        </div>
                        <div className="flex justify-between text-xs">
                            <span>Promise Kept</span>
                            <span className="text-green-400">+{Math.round(((1 - LOSS_RATES_GOOD.bait) - (1 - LOSS_RATES_BAD.bait)) * 100)}% Lift</span>
                        </div>

                        {/* Step 2 */}
                        <div className="relative h-2 bg-gray-800 rounded-full overflow-hidden">
                            <div className="absolute top-0 left-0 h-full bg-red-500" style={{ width: `${(1 - LOSS_RATES_BAD.chaos) * 100}%` }}></div>
                            <div className="absolute top-0 left-0 h-full bg-green-500 opacity-50" style={{ width: `${(1 - LOSS_RATES_GOOD.chaos) * 100}%` }}></div>
                        </div>

                        {/* Step 5 (Final) */}
                        <div className="relative h-8 bg-gray-800 rounded-lg overflow-hidden mt-4">
                            <div className="absolute top-0 left-0 h-full bg-red-600 flex items-center px-2 text-white font-bold" style={{ width: `${(badRevenue / (visitors * conversionValue)) * 100}%` }}>
                                ${badRevenue.toLocaleString()}
                            </div>
                            <div className="absolute top-0 left-0 h-full bg-green-600 flex items-center justify-end px-2 text-white font-bold opacity-80" style={{ width: `${(goodRevenue / (visitors * conversionValue)) * 100}%` }}>
                                ${goodRevenue.toLocaleString()}
                            </div>
                        </div>
                        <div className="text-right text-xs text-green-400">Total Revenue Potential Unlocked</div>
                    </div>
                </div>
                ...
                <div className="space-y-8 flex flex-col justify-center">
                    <div className="space-y-4 bg-white/5 p-6 rounded-xl border border-white/5">
                        <label className="block">
                            <span className="text-gray-400 text-xs uppercase tracking-widest">Adjust Traffic</span>
                            <input
                        ...
                        <label className="block">
                            <span className="text-gray-400 text-xs uppercase tracking-widest">Adjust Order Value</span>
                            <input
                    ...
                    <div className="text-center space-y-4">
                        ...
                        <button
                            onClick={onNext}
                            className="w-full py-5 bg-white text-black font-display font-bold text-xl rounded-full hover:scale-105 transition-transform shadow-[0_0_30px_rgba(255,255,255,0.3)]"
                        >
                            The Verdict &rarr;
                        </button>
                    </div>
            ...
            >
                <div className="bg-white/5 p-6 rounded-2xl border border-white/5 hover:bg-white/10 transition-colors">
                    ...
                    <div className="font-bold text-white mb-2">UX is Math</div>
                ...
                <div className="bg-white/5 p-6 rounded-2xl border border-white/5 hover:bg-white/10 transition-colors">
                    <div className="text-3xl mb-3">📉</div>
                    <div className="font-bold text-pink-400 mb-2">The Hidden Tax</div>
                    <div className="text-sm text-gray-400 leading-relaxed">
                ...
                <div className="bg-white/5 p-6 rounded-2xl border border-white/5 hover:bg-white/10 transition-colors">
                    <div className="text-3xl mb-3">🚀</div>
                    <div className="font-bold text-green-400 mb-2">The Multiplier</div>
                    <div className="text-sm text-gray-400 leading-relaxed">
